...
async def test_task5_endpoints():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        res = await client.post("/api/matches/calculate", json={"business_id": "biz-fashioncart"}, headers=auth_headers)
        ...
        res = await client.post("/api/matches/calculate", json=inline_payload, headers=auth_headers)
        ...
        res = await client.post("/api/matches/calculate", json={}, headers=auth_headers)
        ...
        res = await client.post("/api/matches/calculate", json={"business_id": "non-existent-biz-id"}, headers=auth_headers)
        assert res.status_code == 404, f"Expected 404, got {res.status_code}"
