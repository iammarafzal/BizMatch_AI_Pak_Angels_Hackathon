...
async def generate_explanation_node(state: EvaluationState) -> Dict[str, Any]:
    """
    Generate initial MatchExplanation using Gemini via dual-key failover manager.
    ...
    def chain_builder(llm_instance):
        return prompt | llm_instance.with_structured_output(MatchExplanation)

    def fallback_handler(inps: dict) -> MatchExplanation:
        return _generate_fallback_explanation(business, manager, factor_scores, overall_score)

    explanation = await gemini_manager.execute_with_failover(
