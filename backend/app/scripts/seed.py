...
async def seed_database(reset: bool = False) -> Dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        demo_users = [
            {"email": "founder@bizmatch.ai", "password": "password123", "role": "FOUNDER"},
            {"email": "admin@bizmatch.ai", "password": "admin123", "role": "ADMIN"},
        ]
        for u in demo_users:
            if u["email"] not in existing_user_emails:
                new_u = User(
                    id=str(uuid.uuid4()),
                    email=u["email"],
    ...
    summary_msg = (
        f"Successfully seeded {seeded_managers} managers, {seeded_businesses} businesses, and demo users "
